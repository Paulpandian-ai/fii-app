...
function buildSmartFeed(results: any[]): FeedItem[] {
  ...
  const toFeedItem = (r: any): FeedItem => ({
    id: `sc-${r.ticker}`,
    type: 'signal',
    ticker: r.ticker,
  ...
  const rest = results
    ...
    .sort((a, b) => Math.abs(b.changePercent || 0) - Math.abs(a.changePercent || 0));
  for (const r of rest) add(toFeedItem(r));
    ...
    try {
      ...
      for (const entry of rawFeed) {
        if (entry.type === 'educational') {
          ...
          feedItemMap.set(feedItem.ticker, feedItem);
        }
      }

      // Build smart-ordered cards from screener results
      const screenerResults = screenerData?.results || [];
      if (screenerResults.length > 0) {
        ...
        for (const item of smartFeed) {
          ...
          if (existing) {
            ...
            feedItemMap.delete(item.ticker);
          } else {
