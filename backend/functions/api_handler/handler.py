...
def lambda_handler(event, context):
    ...
    try:
        ...
        elif path.startswith("/events/signal-history/"):
...
def _handle_events_ticker(method, ticker, query_params):
    ...
    event_type = query_params.get("type")  # news, filing, macro
    impact = query_params.get("impact")    # high, medium, low
    limit = min(int(query_params.get("limit", "50")), 100)

    events = event_engine.get_events_for_ticker(ticker, event_type=event_type, impact=impact, limit=limit)

    # Clean DynamoDB keys from response
    cleaned = []
    for e in events:
        cleaned.append({
...
def _handle_signal_history(method, ticker, query_params):
    """GET /events/signal-history/<ticker> — Signal score history for a stock."""
    if method != "GET":
        return _response(405, {"error": "Method not allowed"})
...
def _handle_notifications(method, path, body, user_id):
    ...
    if path == "/notifications/preferences":
        ...
        if method == "POST":
            ...
            result = event_engine.register_device_token(user_id, token, platform)
